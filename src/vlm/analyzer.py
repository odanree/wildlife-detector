...
_COMPARE_USER_PROMPT = (
    ...
    "\n"
    "     Sizing / species — CRITICAL (misclassifying a cat as a mouse\n"
    "     is the top failure mode; err toward the larger species when unsure):\n"
    "       - Shape < 3% of frame width in longest dimension → probably a leaf\n"
    "         piece or insect. Reject unless motion is unambiguous.\n"
    "       - Shape 3-6% of frame width → mouse (species='mouse').\n"
    "       - Shape 6-10% of frame width → rat (species='rat').\n"
    "       - Shape 10-20% of frame width → likely cat OR raccoon\n"
    "         (species='cat' or 'raccoon'). Rodents don't reach this size\n"
    "         in a home yard.\n"
    "       - Shape > 20% of frame width → definitely cat/raccoon/opossum/dog.\n"
    "         NEVER label anything this large as mouse or rat.\n"
    "     For OVERHEAD IR at 2048px-wide input: 60+ px silhouette body-length\n"
    "     is roughly the cat/raccoon boundary. If you see a big dark blob that\n"
    "     could be either 'rat' or 'cat', PREFER cat — human operators can\n"
    "     always downgrade a false-cat to rat, but a silent false-mouse on\n"
    "     a cat means no alert fires at all.\n"
    "     If truly ambiguous (can't tell rat vs cat vs raccoon), use\n"
    "     species='other' rather than 'none'. 'other' is alertable so the\n"
    "     operator sees it; 'none' silently drops it.\n"
    "\n"
